...
$jscomp.polyfill("Array.prototype.fill", function(b) {
    return b ? b : function(b, e, h) {
        var c = this.length || 0;
        0 > e && (e = Math.max(0, c + e));
        if (null == h || h > c) h = c;
        h = Number(h);
        0 > h && (h = Math.max(0, c + h));
        for (e = Number(e || 0); e < h; e++) this[e] = b;
        return this
    }
}, "es6", "es3");
var testing = !1;
...
var tip_pos = -1,
    tip_fr = 1.9;
o.elem.onclick = function() {
    want_play || play_btn.disabled || (want_play = !0, play_btn_click_mtm = Date.now(), play_btn.setEnabled(!1), spinner_shown = nick.disabled = !0, ldmc.style.display = "inline", play_count++)
};
    ...
    redraw = function() {
        ...
        if (animating) {
            ...
            if (high_quality || 0 < gla) {
                ...
                1 != gla && (w = 1 - .25 * gla);
                for (c = foods_c -
                    1; 0 <= c; c--) e = foods[c], e.rx >= fpx1 && e.ry >= fpy1 && e.rx <= fpx2 && e.ry <= fpy2 && (1 == e.rad ? (z = mww2 + gsc * (e.rx - view_xx) - e.fw2, B = mhh2 + gsc * (e.ry - view_yy) - e.fh2, b.globalAlpha = w * e.fr, b.drawImage(e.fi, z, B), b.globalAlpha = h * (.5 + .5 * Math.cos(e.gfr / 13)) * e.fr, b.drawImage(e.fi, z, B)) : (z = mww2 + gsc * (e.rx - view_xx) - e.fw2 * e.rad, B = mhh2 + gsc * (e.ry - view_yy) - e.fh2 * e.rad, b.globalAlpha = w * e.fr, b.drawImage(e.fi, 0, 0, e.fw, e.fh, z, B, e.fw * e.rad, e.fh * e.rad), b.globalAlpha = h * (.5 + .5 * Math.cos(e.gfr / 13)) * e.fr, b.drawImage(e.fi, 0, 0, e.fw, e.fh, z, B, e.fw *
                    e.rad, e.fh * e.rad)))
            } else
                for (c = foods_c - 1; 0 <= c; c--) e = foods[c], e.rx >= fpx1 && e.ry >= fpy1 && e.rx <= fpx2 && e.ry <= fpy2 && (1 == e.rad ? (z = mww2 + gsc * (e.rx - view_xx) - e.fw2, B = mhh2 + gsc * (e.ry - view_yy) - e.fh2, b.globalAlpha = e.fr, b.drawImage(e.fi, z, B)) : (z = mww2 + gsc * (e.rx - view_xx) - e.fw2 * e.rad, B = mhh2 + gsc * (e.ry - view_yy) - e.fh2 * e.rad, b.globalAlpha = e.fr, b.drawImage(e.fi, 0, 0, e.fw, e.fh, z, B, e.fw * e.rad, e.fh * e.rad)));
            b.restore();
            b.save();
            b.globalCompositeOperation = "lighter";
            for (c = preys.length - 1; 0 <= c; c--) {
                ...
                if (-50 <= px && -50 <= py && px <= mwwp50 && py <= mhhp50) {
                    ...
                    1 == h.rad ? (z = px - h.fw2, B = py - h.fh2, b.globalAlpha = .75 * h.fr, b.drawImage(h.fi, z, B), b.globalAlpha = .75 * (.5 + .5 * Math.cos(h.gfr / 13)) * h.fr, b.drawImage(h.fi, z, B)) : (z = px - h.fw2 * h.rad, B = py - h.fh2 * h.rad,
                        b.globalAlpha = .75 * h.fr, b.drawImage(h.fi, 0, 0, h.fw, h.fh, z, B, h.fw * h.rad, h.fh * h.rad), b.globalAlpha = .75 * (.5 + .5 * Math.cos(h.gfr / 13)) * h.fr, b.drawImage(h.fi, 0, 0, h.fw, h.fh, z, B, h.fw * h.rad, h.fh * h.rad))
                }
            }
            b.restore();
            b.save();
            b.strokeStyle = "#90C098";
            for (c = snakes.length - 1; 0 <= c; c--) e = snakes[c], f = e.xx + e.fx, q = e.yy + e.fy + 40, 0 < e.na && f >= bpx1 - 100 && q >= bpy1 && f <= bpx2 + 100 && q <= bpy2 && (e == snake && (e.fnfr++, 200 < e.fnfr && (e.na -= .004, 0 > e.na && (e.na = 0))), b.save(), b.globalAlpha = .5 * e.na * e.alive_amt * (1 - e.dead_amt), b.font = "12px Arial, Helvetica Neue, Helvetica, sans-serif",
                b.fillStyle = e.csw, b.textBaseline = "middle", b.textAlign = "center", h = e.xx + e.fx, w = e.yy + e.fy, h = mww2 + (h - view_xx) * gsc, w = mhh2 + (w - view_yy) * gsc, b.fillText(e.nk, h, w + 32 + 11 * e.sc * gsc), b.restore());
                ...
                if (e = snakes[c], e.iiv) {
                    ...
                    if (1 == render_mode) {
                        ...
                        for (var N = !1, x = e.pts.length - 1; 0 <= x; x--) {
                            ...
                            var z = C.fx,
                                B = C.fy;
                            0 < I && (px += z, py += B, lax = ax, lay = ay, ax = (px + lpx) / 2, ay = (py + lpy) / 2, N || (lax = ax, lay = ay), 1 > I && (t = 1 - I, lpx += (lax - lpx) * t, lpy += (lay - lpy) * t, ax += (lax - ax) * t, ay += (lay - ay) * t), N ? I-- : I -= e.chl + e.fchl, N ? b.quadraticCurveTo(mww2 + (lpx - view_xx) * gsc, mhh2 + (lpy - view_yy) * gsc, mww2 + (ax - view_xx) * gsc, mhh2 + (ay - view_yy) * gsc) : (b.lineTo(mww2 +
                                (ax - view_xx) * gsc, mhh2 + (ay - view_yy) * gsc), N = !0))
                        }
                        b.save();
                        b.lineJoin = "round";
                        b.lineCap = "round";
                        doiosh ? (e.sp > e.fsp && (x = e.alive_amt * (1 - e.dead_amt) * Math.max(0, Math.min(1, (e.sp - e.ssp) / (e.msp - e.ssp))), b.save(), b.strokeStyle = e.cs, b.globalAlpha = .3 * x, b.lineWidth = (E + 6) * gsc, b.stroke(), b.lineWidth = (E + 9) * gsc, b.stroke(), b.lineWidth = (E + 12) * gsc, b.stroke(), b.restore()), b.globalAlpha = 1 * e.alive_amt * (1 - e.dead_amt), b.strokeStyle = "#000000", b.lineWidth = (E + 5) * gsc) : (e.sp > e.fsp && (x = e.alive_amt * (1 - e.dead_amt) * Math.max(0, Math.min(1, (e.sp - e.ssp) / (e.msp - e.ssp))), b.save(), b.lineWidth = (E - 2) * gsc, b.shadowBlur = 30 * gsc, b.shadowColor = "rgba(" + e.rr + "," + e.gg + "," + e.bb + ", " + Math.round(1E4 * x) / 1E4 + ")", b.stroke(), b.stroke(), b.restore()), b.globalAlpha = .4 * e.alive_amt * (1 - e.dead_amt), b.strokeStyle = "#000000", b.lineWidth = (E + 5) * gsc, b.stroke(), b.strokeStyle = e.cs, b.lineWidth = E * gsc, b.strokeStyle = "#000000", b.globalAlpha = 1 * e.alive_amt * (1 - e.dead_amt));
                        b.stroke();
                        b.strokeStyle = e.cs;
                    ...
                    if (2 == render_mode) {
                            ...
                            if (C = e.pts[x], lpx = px, lpy = py, px = C.xx + C.fx, py = C.yy + C.fy, I > -M) {
                                ...
                                for (t = 0; 1 > t; t += M) {
                                    ...
                                    if (K + R < J) K += R;
                                        ...
                                        K = -K;
                                        for (C = (R - K) / J; 1 <= C; C--) K += J, pax = lax + (ax - lax) * K / R, pay = lay + (ay - lay) * K / R, pax >= bpx1 && pay >= bpy1 && pax <= bpx2 && pay <= bpy2 ? (pbx[z] = pax, pby[z] = pay, pbu[z] = 2, B && (G--, 0 >= G ? G = 3 : pbu[z] = 1), f = ax - lax, q = ay - lay, pba[z] = -4 <= f && -4 <= q && 4 > f && 4 > q ? at2lt[32 * q + 128 << 8 | 32 * f + 128] : -8 <= f && -8 <= q && 8 > f && 8 > q ? at2lt[16 * q + 128 << 8 | 16 * f +
                                            128] : -16 <= f && -16 <= q && 16 > f && 16 > q ? at2lt[8 * q + 128 << 8 | 8 * f + 128] : -127 <= f && -127 <= q && 127 > f && 127 > q ? at2lt[q + 128 << 8 | f + 128] : Math.atan2(q, f)) : pbu[z] = 0, z++;
                                        K = R - K
                                    }
                        ...
                        ax >= bpx1 && ay >= bpy1 && ax <= bpx2 && ay <= bpy2 ? (pbu[z] = 2, B && (G--, 0 >= G || (pbu[z] = 1)), pbx[z] = ax, pby[z] = ay, pba[z] = Math.atan2(ay - lay, ax - lax)) : pbu[z] = 0;
                        z++;
                        b.save();
                        ...
                        C = 1;
                        if (e.tsp > e.fsp) {
                            C = e.alive_amt * (1 - e.dead_amt) * Math.max(0, Math.min(1, (e.tsp - e.ssp) /
                                (e.msp - e.ssp)));
                            var L = .37 * C;
                            M = Math.pow(C, .5);
                            ...
                            B && (K = 12);
                            if (e.rbcs)
                                for (A = e.rbcs, G = A.length, x = z - 1; 0 <= x; x--) 2 == pbu[x] && (px = pbx[x], py = pby[x], q = per_color_imgs[A[x % G]], q = q.kfmc, b.save(), b.globalAlpha = I * M * .38 * (.6 + .4 * Math.cos(x / K - 1.15 * e.sfr)), b.translate((px - view_xx) * gsc, (py - view_yy) * gsc), 4 > x ? (f = J * (1 + (4 - x) * e.swell), b.drawImage(q, -f, -f, 2 * f, 2 * f)) : b.drawImage(q, -J, -J, 2 * J, 2 * J), b.restore());
                            else
                                for (x = z - 1; 0 <= x; x--) 2 == pbu[x] && (px =
                                    pbx[x], py = pby[x], b.save(), b.globalAlpha = I * M * .38 * (.6 + .4 * Math.cos(x / K - 1.15 * e.sfr)), b.translate((px - view_xx) * gsc, (py - view_yy) * gsc), 4 > x ? (f = J * (1 + (4 - x) * e.swell), b.drawImage(q, -f, -f, 2 * f, 2 * f)) : b.drawImage(q, -J, -J, 2 * J, 2 * J), b.restore());
                            b.restore();
                            C = 1 - C
                        }
                        C *= I;
                        if (high_quality || 0 < gla)
                            for (x = C, 1 != gla && (x = C * gla), b.globalAlpha = x, x = z - 1; 0 <= x; x--) 2 == pbu[x] && (px = pbx[x], py = pby[x], b.save(), b.translate((px - view_xx) * gsc, (py - view_yy) * gsc), b.drawImage(komc, -t, -t, 2 * t, 2 * t), 9 > x && (b.globalAlpha = I * (1 - x / 9), 4 > x ? (f = H * (1 + (4 - x) * e.swell),
                                b.drawImage(ksmc, -f, -f, 2 * f, 2 * f)) : b.drawImage(ksmc, -H, -H, 2 * H, 2 * H), b.globalAlpha = I), b.restore());
                        b.globalAlpha = C;
                        if (e.rbcs) {
                            A = e.rbcs;
                            C = e.fdhc;
                            ...
                            O = e.fdl;
                            G = A.length;
                            for (x = z - 1; 0 <= x; x--) 1 <= pbu[x] && (px = pbx[x], py = pby[x], 4 <= x && (t = x - 4, 2 == pbu[t] && (f = pbx[t], q = pby[t], b.save(), b.translate((f - view_xx) * gsc, (q - view_yy) * gsc), 9 > t ? (b.globalAlpha = t / 9 * I, 4 > t ? (f = H * (1 + (4 - t) * e.swell), b.drawImage(ksmc, -f, -f, 2 * f, 2 * f)) : b.drawImage(ksmc, -H, -H, 2 * H, 2 * H)) : (b.globalAlpha = I, b.drawImage(ksmc, -H, -H, 2 * H, 2 * H)), b.restore())), b.save(),
                                b.globalAlpha = I, b.translate((px - view_xx) * gsc, (py - view_yy) * gsc), nsr || b.rotate(pba[x]), t = 0, e.cusk || (S ? (t = x % P, t >= N && (t = P - (t + 1))) : t = x % N), q = per_color_imgs[A[x % G]], 4 > x ? (f = E * (1 + (4 - x) * e.swell), b.drawImage(q.kmcs[t], -gsc * f, -gsc * f, 2 * gsc * f, 2 * gsc * f)) : b.drawImage(q.kmcs[t], -gsc * E, -gsc * E, 2 * gsc * E, 2 * gsc * E), C && x < O && (t = 1 - x / O, q = per_color_imgs[C], b.globalAlpha = I * t, f = (1 + .05 * t) * E * (1 + (4 - x) * e.swell), b.drawImage(q.kmcs[0], -gsc * f, -gsc * f, 2 * gsc * f, 2 * gsc * f)), F && x > z - O && (t = 1 - (z - x) / O, q = per_color_imgs[F], b.globalAlpha = I * t, f = (1 + .05 *
                                    t) * E * (1 + (4 - x) * e.swell), b.drawImage(q.kmcs[0], -gsc * f, -gsc * f, 2 * gsc * f, 2 * gsc * f)), b.restore());
                            if (e.tsp > e.fsp && (high_quality || 0 < gla)) {
                                ...
                                B && (K = 12);
                                J = 2 * E;
                                for (x = z - 1; 0 <= x; x--) 2 == pbu[x] && (px = pbx[x], py = pby[x], b.save(), b.translate((px - view_xx) * gsc, (py - view_yy) * gsc), b.globalAlpha = I * L * gla * (.5 + .5 * Math.cos(x / K - e.sfr)), 4 > x ? (f = J * (1 + (4 - x) * e.swell), b.drawImage(per_color_imgs[A[x % G]].kfmc, -gsc * f, -gsc * f, 2 * gsc * f, 2 * gsc * f)) : b.drawImage(per_color_imgs[A[x % G]].kfmc, -gsc * J, -gsc *
                                    J, 2 * gsc * J, 2 * gsc * J), b.restore());
                                b.restore()
                            }
                        } else {
                            for (x = z - 1; 0 <= x; x--) 1 <= pbu[x] && (px = pbx[x], py = pby[x], 4 <= x && (t = x - 4, 2 == pbu[t] && (f = pbx[t], q = pby[t], b.save(), b.translate((f - view_xx) * gsc, (q - view_yy) * gsc), 9 > t ? (b.globalAlpha = t / 9 * I, 4 > t ? (f = H * (1 + (4 - t) * e.swell), b.drawImage(ksmc, -f, -f, 2 * f, 2 * f)) : b.drawImage(ksmc, -H, -H, 2 * H, 2 * H)) : (b.globalAlpha = I, b.drawImage(ksmc, -H, -H, 2 * H, 2 * H)), b.restore())), b.save(), b.globalAlpha = I, b.translate((px - view_xx) * gsc, (py - view_yy) * gsc), nsr || b.rotate(pba[x]), t = 0, e.cusk || (S ? (t = x % P, t >= N &&
                                (t = P - (t + 1))) : t = x % N), 4 > x ? (f = E * (1 + (4 - x) * e.swell), b.drawImage(O[t], -gsc * f, -gsc * f, 2 * gsc * f, 2 * gsc * f)) : b.drawImage(O[t], -gsc * E, -gsc * E, 2 * gsc * E, 2 * gsc * E), b.restore());
                            if (e.tsp > e.fsp && (high_quality || 0 < gla)) {
                                b.save();
                                b.globalCompositeOperation = "lighter";
                                K = 4;
                                B && (K = 12);
                                J = 2 * E;
                                for (x = z - 1; 0 <= x; x--) 1 <= pbu[x] && (px = pbx[x], py = pby[x], b.save(), b.translate((px - view_xx) * gsc, (py - view_yy) * gsc), b.globalAlpha = I * L * gla * (.5 + .5 * Math.cos(x / K - e.sfr)), 4 > x ? (f = J * (1 + (4 - x) * e.swell), b.drawImage(F.kfmc, -gsc * f, -gsc * f, 2 * gsc * f, 2 * gsc * f)) : b.drawImage(F.kfmc, -gsc * J, -gsc * J, 2 * gsc * J, 2 * gsc * J), b.restore());
                                b.restore()
                            }
                        }
                        if (e.antenna)
                            if (f = Math.cos(e.ang), q = Math.sin(e.ang), ax = h - 8 * f * e.sc, ay = w - 8 * q * e.sc, 2 <= z && ax >= apx1 && ay >= apy1 && ax <= apx2 && ay <= apy2) {
                                ...
                                for (x = 1; x <= fj; x++) xx = e.atx[x - 1], yy = e.aty[x - 1], xx += 2 * Math.random() - 1, yy +=
                                    2 * Math.random() - 1, f = e.atx[x] - xx, q = e.aty[x] - yy, ang = -4 <= f && -4 <= q && 4 > f && 4 > q ? at2lt[32 * q + 128 << 8 | 32 * f + 128] : -8 <= f && -8 <= q && 8 > f && 8 > q ? at2lt[16 * q + 128 << 8 | 16 * f + 128] : -16 <= f && -16 <= q && 16 > f && 16 > q ? at2lt[8 * q + 128 << 8 | 8 * f + 128] : -127 <= f && -127 <= q && 127 > f && 127 > q ? at2lt[q + 128 << 8 | f + 128] : Math.atan2(q, f), xx += 4 * Math.cos(ang) * e.sc, yy += 4 * Math.sin(ang) * e.sc, e.atvx[x] += .1 * (xx - e.atx[x]), e.atvy[x] += .1 * (yy - e.aty[x]), e.atx[x] += e.atvx[x], e.aty[x] += e.atvy[x], e.atvx[x] *= .88, e.atvy[x] *= .88, f = e.atx[x] - e.atx[x - 1], q = e.aty[x] - e.aty[x - 1], R = Math.sqrt(f *
                                        f + q * q), R > 4 * e.sc && (ang = -4 <= f && -4 <= q && 4 > f && 4 > q ? at2lt[32 * q + 128 << 8 | 32 * f + 128] : -8 <= f && -8 <= q && 8 > f && 8 > q ? at2lt[16 * q + 128 << 8 | 16 * f + 128] : -16 <= f && -16 <= q && 16 > f && 16 > q ? at2lt[8 * q + 128 << 8 | 8 * f + 128] : -127 <= f && -127 <= q && 127 > f && 127 > q ? at2lt[q + 128 << 8 | f + 128] : Math.atan2(q, f), e.atx[x] = e.atx[x - 1] + 4 * Math.cos(ang) * e.sc, e.aty[x] = e.aty[x - 1] + 4 * Math.sin(ang) * e.sc);
                                b.globalAlpha = I;
                                ...
                                1 <= Math.abs(xx - f) + Math.abs(yy - q) && (f = xx, q = yy, b.lineTo(f, q));
                                b.stroke();
                                ...
                                q = (e.aty[fj] - view_yy) * gsc;
                                b.moveTo(f, q);
                                for (x = fj - 1; 0 <= x; x--) xx = (e.atx[x] - view_xx) * gsc,
                                    yy = (e.aty[x] - view_yy) * gsc, 1 <= Math.abs(xx - f) + Math.abs(yy - q) && (f = xx, q = yy, b.lineTo(f, q));
                                b.stroke();
                                ...
                                if (e.abrot) {
                                    ...
                                    b.rotate(e.atba);
                                    b.drawImage(e.bulb, e.blbx * e.bsc * C, e.blby *
                                        e.bsc * C, e.blbw * e.bsc * C, e.blbh * e.bsc * C);
                                    b.restore()
                            ...
                            } else e.antenna_shown && (e.antenna_shown = !1);
                        if (e.dead) {
                            b.save();
                            b.globalCompositeOperation = "lighter";
                            B = (.15 + .15 * Math.abs(Math.sin(5 * Math.PI * e.dead_amt))) * Math.sin(Math.PI * e.dead_amt);
                            E *= gsc;
                            for (x = z - 1; 0 <= x; x--) 2 == pbu[x] && (px = pbx[x],
                                py = pby[x], b.save(), b.globalAlpha = B * (.6 + .4 * Math.cos(x / 4 - 15 * e.dead_amt)), b.translate((px - view_xx) * gsc, (py - view_yy) * gsc), 4 > x ? (f = E * (1 + (4 - x) * e.swell), b.drawImage(kdmc, -f, -f, 2 * f, 2 * f)) : b.drawImage(kdmc, -E, -E, 2 * E, 2 * E), b.restore());
                            b.restore()
                        }
                        b.restore()
                    }
                    e.one_eye ? (E = 3 * y, x = Math.cos(D) * E, z = Math.sin(D) * E, B = y * e.ebisz, b.drawImage(e.ebi, 0, 0, e.ebiw, e.ebih, mww2 + (x + h - B / 2 - view_xx) * gsc, mhh2 + (z + w - B / 2 - view_yy) * gsc, B * gsc, B * gsc), x = Math.cos(D) * (E + .15) + e.rex * y, z = Math.sin(D) * (E + .15) + e.rey * y, B = y * e.episz, b.drawImage(e.epi,
                        0, 0, e.epiw, e.epih, mww2 + (x + h - B / 2 - view_xx) * gsc, mhh2 + (z + w - B / 2 - view_yy) * gsc, B * gsc, B * gsc)) : (E = e.ed * y, B = e.esp * y, e.eac || (x = Math.cos(D) * E + Math.cos(D - Math.PI / 2) * (B + .5), z = Math.sin(D) * E + Math.sin(D - Math.PI / 2) * (B + .5), b.fillStyle = e.ec, 0 < e.eo && (b.lineWidth = e.eo * gsc, b.strokeStyle = "#000000"), b.globalAlpha = e.eca * e.alive_amt, b.beginPath(), b.arc(mww2 + (x + h - view_xx) * gsc, mhh2 + (z + w - view_yy) * gsc, e.er * y * gsc, 0, pi2), b.closePath(), 0 < e.eo && b.stroke(), b.fill(), b.globalAlpha = e.ppa, x = Math.cos(D) * (E + .5) + e.rex * y + Math.cos(D - Math.PI /
                        2) * B, z = Math.sin(D) * (E + .5) + e.rey * y + Math.sin(D - Math.PI / 2) * B, b.fillStyle = e.ppc, b.beginPath(), b.arc(mww2 + (x + h - view_xx) * gsc, mhh2 + (z + w - view_yy) * gsc, e.pr * y * gsc, 0, pi2), b.closePath(), b.fill()), e.eac || (x = Math.cos(D) * E + Math.cos(D + Math.PI / 2) * (B + .5), z = Math.sin(D) * E + Math.sin(D + Math.PI / 2) * (B + .5), b.fillStyle = e.ec, 0 < e.eo && (b.lineWidth = e.eo * gsc, b.strokeStyle = "#000000"), b.globalAlpha = e.eca * e.alive_amt, b.beginPath(), b.arc(mww2 + (x + h - view_xx) * gsc, mhh2 + (z + w - view_yy) * gsc, e.er * y * gsc, 0, pi2), b.closePath(), 0 < e.eo && b.stroke(),
                        b.fill(), b.globalAlpha = e.ppa, x = Math.cos(D) * (E + .5) + e.rex * y + Math.cos(D + Math.PI / 2) * B, z = Math.sin(D) * (E + .5) + e.rey * y + Math.sin(D + Math.PI / 2) * B, b.fillStyle = e.ppc, b.beginPath(), b.arc(mww2 + (x + h - view_xx) * gsc, mhh2 + (z + w - view_yy) * gsc, e.pr * y * gsc, 0, pi2), b.closePath(), b.fill()), e.jyt && (C = e.sc * gsc * .25, E = -3 * y, B = 7 * y, x = Math.cos(D) * (E + .5) + e.rex * y + Math.cos(D - Math.PI / 2) * B, z = Math.sin(D) * (E + .5) + e.rey * y + Math.sin(D - Math.PI / 2) * B, b.save(), b.translate(mww2 + (x + h - view_xx) * gsc, mhh2 + (z + w - view_yy) * gsc), b.rotate(D), b.drawImage(ecmc, -24 *
                        C, -24 * C, 48 * C, 48 * C), b.restore(), x = Math.cos(D) * (E + .5) + e.rex * y + Math.cos(D + Math.PI / 2) * B, z = Math.sin(D) * (E + .5) + e.rey * y + Math.sin(D + Math.PI / 2) * B, b.save(), b.translate(mww2 + (x + h - view_xx) * gsc, mhh2 + (z + w - view_yy) * gsc), b.rotate(D), b.drawImage(ecmc, -24 * C, -24 * C, 48 * C, 48 * C), b.restore(), E = 5 * y, x = Math.cos(D) * (E + .5) + e.rex * y, z = Math.sin(D) * (E + .5) + e.rey * y, C = e.sc * gsc * .16, b.save(), b.translate(mww2 + (x + h - view_xx) * gsc, mhh2 + (z + w - view_yy) * gsc), b.rotate(D), b.drawImage(jmou, -40 * C, -65 * C, 79 * C, 130 * C), b.restore()));
                    b.globalAlpha = 1;
                    e.slg &&
                        (C = e.sc * gsc * .25, b.save(), f = 13 * Math.cos(D) * y + Math.cos(D - Math.PI / 2) * (6 * y + .5), q = 13 * Math.sin(D) * y + Math.sin(D - Math.PI / 2) * (6 * y + .5), b.translate(mww2 + (f + h - view_xx) * gsc, mhh2 + (q + w - view_yy) * gsc), b.rotate(D - .4), b.drawImage(sest, -28 * C, -44 * C, 105 * C, 88 * C), b.restore(), b.save(), f = 13 * Math.cos(D) * y + Math.cos(D + Math.PI / 2) * (6 * y + .5), q = 13 * Math.sin(D) * y + Math.sin(D + Math.PI / 2) * (6 * y + .5), b.translate(mww2 + (f + h - view_xx) * gsc, mhh2 + (q + w - view_yy) * gsc), b.rotate(D + .4), b.drawImage(sest, -28 * C, -44 * C, 105 * C, 88 * C), b.restore())
                }
            if (high_quality ||
                0 < gla) {
                b.save();
                b.globalCompositeOperation = "lighter";
                for (c = foods_c - 1; 0 <= c; c--) e = foods[c], e.rx >= fpx1 && e.ry >= fpy1 && e.rx <= fpx2 && e.ry <= fpy2 && (f = e.rx - view_xx, q = e.ry - view_yy, h = f * f + q * q, fs = 1 + .06 * e.rad, z = f * fs, B = q * fs, L = .005 + .09 * (1 - h / (86E3 + h)), 1 != e.rad && (L *= Math.pow(e.rad, .25)), 1 != gla && (L *= gla), z = z * gsc + mww2, B = B * gsc + mhh2, 1 == e.rad ? (z -= e.gfw2, B -= e.gfh2, b.globalAlpha = L * e.fr, b.drawImage(e.gfi, z, B), b.globalAlpha = L * (.5 + .5 * Math.cos(e.gfr / 13)) * e.fr, b.drawImage(e.gfi, z, B)) : (z -= e.gfw2 * e.rad, B -= e.gfh2 * e.rad, b.globalAlpha =
                    L * e.fr, b.drawImage(e.gfi, 0, 0, e.gfw, e.gfh, z, B, e.gfw * e.rad, e.gfh * e.rad), b.globalAlpha = L * (.5 + .5 * Math.cos(e.gfr / 13)) * e.fr, b.drawImage(e.gfi, 0, 0, e.gfw, e.gfh, z, B, e.gfw * e.rad, e.gfh * e.rad)), fs = 1 + .32 * e.rad, z = f * fs, B = q * fs, L = .085 * (1 - h / (16500 + h)), 1 != e.rad && (L *= Math.pow(e.rad, .25)), 1 != gla && (L *= gla), z = z * gsc + mww2, B = B * gsc + mhh2, 1 == e.rad ? (z -= e.g2fw2, B -= e.g2fh2, b.globalAlpha = L * e.fr, b.drawImage(e.g2fi, z, B), b.globalAlpha = L * (.5 + .5 * Math.cos(e.gfr / 13)) * e.fr, b.drawImage(e.g2fi, z, B)) : (z -= e.g2fw2 * e.rad, B -= e.g2fh2 * e.rad, b.globalAlpha =
                    L * e.fr, b.drawImage(e.g2fi, 0, 0, e.g2fw, e.g2fh, z, B, e.g2fw * e.rad, e.g2fh * e.rad), b.globalAlpha = L * (.5 + .5 * Math.cos(e.gfr / 13)) * e.fr, b.drawImage(e.g2fi, 0, 0, e.g2fw, e.g2fh, z, B, e.g2fw * e.rad, e.g2fh * e.rad)));
                b.restore()
            }
            b.save();
            b.globalCompositeOperation = "lighter";
            for (c = preys.length - 1; 0 <= c; c--) h = preys[c], f = h.xx + h.fx, q = h.yy + h.fy, h.eaten && (e = h.eaten_by, t = Math.pow(h.eaten_fr, 2), f += (e.xx + e.fx + Math.cos(e.ang + e.fa) * (43 - 24 * t) * (1 - t) - f) * t, q += (e.yy + e.fy + Math.sin(e.ang + e.fa) * (43 - 24 * t) * (1 - t) - q) * t), f -= view_xx, q -= view_yy, e =
                f * f + q * q, fs = 1 + .08 * h.rad, px = f * fs, py = q * fs, L = .4 * (1 - e / (176E3 + e)), 1 != h.rad && (L *= Math.pow(h.rad, .25)), px = px * gsc + mww2, py = py * gsc + mhh2, 1 == h.rad ? -150 <= px && -150 <= py && px <= mwwp150 && py <= mhhp150 && (px -= h.gfw2, py -= h.gfh2, b.globalAlpha = L * h.fr, b.drawImage(h.gfi, px, py), b.globalAlpha = L * (.5 + .5 * Math.cos(h.gfr / 13)) * h.fr, b.drawImage(h.gfi, px, py)) : -150 <= px && -150 <= py && px <= mwwp150 && py <= mhhp150 && (px -= h.gfw2 * h.rad, py -= h.gfh2 * h.rad, b.globalAlpha = L * h.fr, b.drawImage(h.gfi, 0, 0, h.gfw, h.gfh, px, py, h.gfw * h.rad, h.gfh * h.rad), b.globalAlpha =
                    L * (.5 + .5 * Math.cos(h.gfr / 13)) * h.fr, b.drawImage(h.gfi, 0, 0, h.gfw, h.gfh, px, py, h.gfw * h.rad, h.gfh * h.rad)), fs = 1 + .32 * h.rad, px = f * fs, py = q * fs, L = .35 * (1 - e / (46500 + e)), 1 != h.rad && (L *= Math.pow(h.rad, .25)), e = 2 * h.rad, px = px * gsc + mww2, py = py * gsc + mhh2, -150 <= px && -150 <= py && px <= mwwp150 && py <= mhhp150 && (px -= h.gfw2 * e, py -= h.gfh2 * e, b.globalAlpha = L * h.fr, b.drawImage(h.gfi, 0, 0, h.gfw, h.gfh, px, py, h.gfw * e, h.gfh * e), b.globalAlpha = L * (.5 + .5 * Math.cos(h.gfr / 13)) * h.fr, b.drawImage(h.gfi, 0, 0, h.gfw, h.gfh, px, py, h.gfw * e, h.gfh * e));
            b.restore();
            if (4E3 >
                Math.abs(grd - view_dist)) {
                b.save();
                b.lineWidth = 23 * gsc;
            ...
            wumsts && 0 < rank && 0 < snake_count && playing && (wumsts = !1, e = "Your length", c = "of", L = "Your rank", "de" == lang ? (e = "Deine L\u00e4nge", c = "von", L = "Dein rang") : "fr" == lang ? (e = "Votre longueur", c = "de", L = "Ton rang") : "pt" == lang && (e = "Seu comprimento", c = "do", L = "Seu classifica\u00e7\u00e3o"), e = '<span style="font-size: 14px;"><span style="opacity: .4;">' +
                e + ': </span><span style="opacity: .8; font-weight: bold;">' + Math.floor(15 * (fpsls[snake.sct] + snake.fam / fmlts[snake.sct] - 1) - 5) / 1 + "</span></span>", e += '<BR><span style="opacity: .3;">' + L + ': </span><span style="opacity: .35;">' + rank + '</span><span style="opacity: .3;"> ' + c + ' </span><span style="opacity: .35;">' + snake_count + "</span>", lbf.innerHTML = e);
            b.restore()
        }
    };
dfe = "e" + dfe;
...
function resize() {
    ...
    if (ww != lww || hh != lhh) {
        ...
        testing && (pskh.style.left = Math.round(.05 * ww - 44) + "px", nskh.style.left = Math.round(.95 * ww - 44) + "px");
        reposSkinStuff();
        ...
        if (mww != b || mhh != e) mww = b,
            mhh = e, mc.width = mww, mc.height = mhh, mwwp50 = mww + 50, mhhp50 = mhh + 50, mwwp150 = mww + 150, mhhp150 = mhh + 150, mww2 = mww / 2, mhh2 = mhh / 2, rdgbg();
...
function reposSkinStuff() {
    ...
    testing && (skodiv.style.top = Math.round(.8 * hh + 120 + 54 * skoboym) + "px");
            ...
            for (h = 2; h < f.length; h += 4) {
                ...
                for (var K = 1; K <= q; K++) {
                    ...
                    t +=
                        D;
                    lctx.beginPath();
                    T = 1.15 * I * Math.min(1, lgsc * (.2 + .8 * lga) * (30 * lgss * C - B / 20 - c / 2));.5 < T && (lctx.arc(O * lgsc, P * lgsc, T, 0, pi2), w.wch = !0);
                    lctx.fill()
                }
            }
        }
